...
def jsproxy(path):
	...
	if request.method == "POST":
		r = requests.post("http://"+path, data=request.form, headers=request.headers)
		return Response(r.content.replace("//platform.launchrock.com/v1/","//localhost:5000/json/platform.launchrock.com/v1/"),  mimetype='application/javascript')
	else:
		print path
		...
		print path[-4:]
		if path[-4:] == ".css":
			return Response(r.content.replace("//platform.launchrock.com/v1/","//localhost:5000/json/platform.launchrock.com/v1/"),mimetype="text/css")
		else:
			return Response(r.content.replace("//platform.launchrock.com/v1/","//localhost:5000/json/platform.launchrock.com/v1/"),mimetype="application/javascript")
